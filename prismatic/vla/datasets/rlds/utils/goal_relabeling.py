...
def identity(traj: Dict) -> Dict:
    """Relabels with the final observation as the goal."""
    traj_len = tf.shape(tf.nest.flatten(traj["observation"])[0])[0]

    # Select the final observation as the goal for each transition
    goal_idxs = tf.broadcast_to(traj_len - 1, [traj_len])
