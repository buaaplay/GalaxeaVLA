...
# ruff: noqa: B023
def normalize_action_and_proprio(traj: Dict, 
                                 metadata: Dict, 
    ...
    if action_encoding in [ActionEncoding.EEF_POS, ActionEncoding.JOINT_POS, 
        ...
        # rescale gripper action to [-1, 1]
        # single arm, gripper at last dimension
        action_len = traj["action"].shape[-1]
        mask = [False] * (action_len - 1) + [True]
        traj["action"] = tf.where(mask, traj["action"] * 2 - 1.0, traj["action"])
        
    elif action_encoding in [ActionEncoding.JOINT_POS_BIMANUAL, ActionEncoding.ABS_EEF_POS_BIMANUAL, 
                             ActionEncoding.REL_EEF_POS_BIMANUAL]:
        # rescale gripper action to [-1, 1]
        # bimanual, gripper at last dimension on each arm
        action_len = traj["action"].shape[-1]
        mask = [False] * 6 + [True] + [False] * 6 + [True]
        traj["action"] = tf.where(mask, traj["action"] * 2 - 1.0, traj["action"])
    elif action_encoding in [ActionEncoding.R1_LITE, ActionEncoding.R1_LITE_RELJOINT]:
        # rescale gripper action to [-1, 1]
        mask = [False] * 6 + [True] + [False] * 6 + [True] + [False] * 12
        traj["action"] = tf.where(mask, traj["action"] * 2 - 1.0, traj["action"])
    else:
        raise ValueError(f"Unknown action encoding {action_encoding}")
...
def get_dataset_statistics(
    full_dataset: dl.DLataset,
    train_dataset: dl.DLataset,
    val_dataset: dl.DLataset,
    ...
    # Computing statistics for train and val splits
    num_train_transitions, num_train_trajectories = 0, 0
    for traj in train_dataset.iterator():
        num_train_transitions += traj["action"].shape[0]
        num_train_trajectories += 1
    
    metadata["num_train_transitions"] = num_train_transitions
    metadata["num_train_trajectories"] = num_train_trajectories

    num_val_transitions, num_val_trajectories = 0, 0
    for traj in val_dataset.iterator():
        num_val_transitions += traj["action"].shape[0]
        num_val_trajectories += 1
    
    metadata["num_val_transitions"] = num_val_transitions
    metadata["num_val_trajectories"] = num_val_trajectories
...
def recursive_to_serializable(stats):
    if isinstance(stats, dict):
        for k, v in stats.items():
            stats[k] = recursive_to_serializable(v)
    else:
        if isinstance(stats, np.ndarray):
            stats = stats.tolist()
        elif isinstance(stats, np.integer):
            stats = int(stats)
...
def save_dataset_statistics(dataset_statistics, run_dir):
    ...
    with open(out_path, "w") as f_json:
        dataset_statistics = recursive_to_serializable(dataset_statistics)
