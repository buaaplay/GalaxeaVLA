...
def repeat_last_timestep(traj: Dict, repeat_ratio: float = 0.0) -> Dict:
    """
    Repeats the last timestep of the trajectory with a probability of `repeat_ratio`.
    Should be applied after chunk_act_obs.
    """

    def pad_tensor(t: tf.Tensor, pad_count: int) -> tf.Tensor:
        # pads the last element of t with pad_count copies of the last element
        last = t[-1]
        # expand_dims -> [1, ...]，repeat pad_count -> [pad_steps, ...]
        pad = tf.repeat(tf.expand_dims(last, 0), repeats=pad_count, axis=0)
        
        return tf.concat([t, pad], axis=0)

    traj_len = tf.shape(traj["action"])[0]
    repeat_count = tf.cast(traj_len, tf.float32) * repeat_ratio
    repeat_count = tf.cast(tf.math.ceil(repeat_count), tf.int32)

    # 1) pad observations
    traj["observation"] = tf.nest.map_structure(lambda x: pad_tensor(x, repeat_count), traj["observation"])

    # 2) pad actions
    traj["action"] = pad_tensor(traj["action"], repeat_count)

    # 3) pad tasks
    if "task" in traj:
        traj["task"] = tf.nest.map_structure(lambda x: pad_tensor(x, repeat_count), traj["task"])

    # other keys
    for key in traj:
        if key not in ["observation", "action", "task"]:
            traj[key] = pad_tensor(traj[key], repeat_count)
...
def chunk_act_obs(traj: Dict, window_size: int, future_action_window_size: int = 0) -> Dict:
    ...
    if "task" in traj and "timestep" in traj["task"]:
    ...
    # assign padding mask for the padded actions
    traj["action_pad_mask"] = action_past_goal
