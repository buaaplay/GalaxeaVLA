...
from prismatic.vla.datasets.rlds.utils.encoding import StateEncoding, ActionEncoding
...
def galaxea_r1_dataset_transform(trajectory: Dict[str, Any]) -> Dict[str, Any]:
    EEF_DIM = 6 # XYZ + RPY

    # gripper action is in 0 (close)...100 (open) --> scale --> +1 = open, -1 = close
    action_gripper_left = trajectory["action"][:, EEF_DIM: EEF_DIM+1] # 1-dim
    action_gripper_right = trajectory["action"][:, 2*EEF_DIM+1: 2*EEF_DIM+2] # 1-dim

    # moved to data_utils.normalize_action_and_proprio
    action_gripper_left_normalized = tf.clip_by_value((action_gripper_left - 0) / (100 - 0), 0, 1) 
    action_gripper_right_normalized = tf.clip_by_value((action_gripper_right - 0) / (100 - 0), 0, 1) 

    trajectory["action"] = tf.concat(
        [
            trajectory["action"][:, :EEF_DIM],
            action_gripper_left_normalized,
            trajectory["action"][:, EEF_DIM+1: 2*EEF_DIM+1],
            action_gripper_right_normalized,
        ],
        axis=1,
    )
...
def galaxea_r1_lite_dataset_transform(trajectory: Dict[str, Any]) -> Dict[str, Any]:
    QPOS_DIM = 6

    # gripper action is in 0 (close)...100 (open) --> scale --> +1 = open, -1 = close
    action_gripper_left = trajectory["action"][:, QPOS_DIM: QPOS_DIM + 1] # 1-dim
    action_gripper_right = trajectory["action"][:, 2 * QPOS_DIM + 1: 2 * QPOS_DIM + 2] # 1-dim
    ...
    trajectory["action"] = tf.concat(
        [
            trajectory["action"][:, :QPOS_DIM],
            action_gripper_left_normalized,
            trajectory["action"][:, QPOS_DIM + 1: 2 * QPOS_DIM + 1],
            action_gripper_right_normalized,
            trajectory["action"][:, 2 * QPOS_DIM + 2:]
        ],
        axis=1,
    )
...
OXE_STANDARDIZATION_TRANSFORMS = {
    ...
    "bridgev2_depth_intrinsics": bridge_orig_dataset_transform,
    ...
    "libero_spatial_no_noops_depth_intrinsics": libero_dataset_transform,
    "libero_object_no_noops_depth_intrinsics": libero_dataset_transform,
    "libero_goal_no_noops_depth_intrinsics": libero_dataset_transform,
    "libero_10_no_noops_depth_intrinsics": libero_dataset_transform,
    ### galaxea datasets v2
    "galaxea_r1": galaxea_r1_dataset_transform,
    "galaxea_r1_lite": galaxea_r1_lite_dataset_transform,
}
