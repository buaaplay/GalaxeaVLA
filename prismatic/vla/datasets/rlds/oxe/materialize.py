...
def make_oxe_dataset_kwargs(
    ...
    # hardcode for galaxea data
    if "r1_lite" in dataset_name:
        dataset_kwargs = deepcopy(OXE_DATASET_CONFIGS["galaxea_r1_lite"])
        if use_relative_joint_action:
            dataset_kwargs["action_encoding"] = ActionEncoding.R1_LITE_RELJOINT
    elif "galaxea" in dataset_name:
        dataset_kwargs = deepcopy(OXE_DATASET_CONFIGS["galaxea_r1"])
    else:
    ...
    # `absolute_action_mask` indicates whether the action dim is absolute or delta value over the previous action dim
    ...
    elif dataset_kwargs["action_encoding"] is ActionEncoding.ABS_EEF_POS_BIMANUAL:
        # [Contract for Galaxea dataset] Bimanual actions are split into two halves
        dataset_kwargs["absolute_action_mask"] = 2 * ([True] * 6 + [True])
        dataset_kwargs["action_normalization_mask"] = 2 * ([True] * 6 + [False])
    elif dataset_kwargs["action_encoding"] is ActionEncoding.REL_EEF_POS_BIMANUAL:
        # [Contract for Galaxea dataset] Bimanual actions are split into two halves
        dataset_kwargs["absolute_action_mask"] = 2 * ([False] * 6 + [True])
        dataset_kwargs["action_normalization_mask"] = 2 * ([True] * 6 + [False])
    elif dataset_kwargs["action_encoding"] is ActionEncoding.R1_LITE:
        dataset_kwargs["absolute_action_mask"] = 2 * ([True] * 6 + [True]) + [False] * 12
        dataset_kwargs["action_normalization_mask"] = 2 * ([True] * 6 + [False]) + [True] * 12
    elif dataset_kwargs["action_encoding"] is ActionEncoding.R1_LITE_RELJOINT:
        dataset_kwargs["absolute_action_mask"] = 2 * ([False] * 6 + [True]) + [False] * 12
        dataset_kwargs["action_normalization_mask"] = 2 * ([True] * 6 + [False]) + [True] * 12
    else:
        raise ValueError(f"Cannot load `{dataset_name}`, invalid action encoding `{dataset_kwargs['action_encoding']}`")
    ...
    if use_last_action:
        dataset_kwargs["state_obs_keys"].append("last_action")
    ...
    # hardcode for galaxea data
    if "r1_lite" in dataset_name:
        dataset_kwargs["standardize_fn"] = OXE_STANDARDIZATION_TRANSFORMS["galaxea_r1_lite"]
    elif "galaxea" in dataset_name:
        dataset_kwargs["standardize_fn"] = OXE_STANDARDIZATION_TRANSFORMS["galaxea_r1"]
    else:
    ...
    for d_name, d_weight in filtered_mixture_spec:
        try:
            ...
            if max_trajectories is not None:
                dataset_kwargs["max_trajectories"] = max_trajectories

            per_dataset_kwargs.append(dataset_kwargs)
            sampling_weights.append(d_weight)

        except ValueError as e:
            overwatch.warning(f"Skipping `{d_name}` due to Value Error: {e}")
        # except KeyError as e:
        #     per_dataset_kwargs.append(
        #         {"name": d_name, "data_dir": str(data_root_dir)}
        #     )
        #     sampling_weights.append(0)
        #     overwatch.warning(f"Skipping `{d_name}` due to KeyError: {e}")
