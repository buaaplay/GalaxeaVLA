...
class FSDPStrategy(TrainingStrategy):
    ...
    def save_checkpoint(
        ...
        with FSDP.state_dict_type(self.vlm, self.fsdp_state_dict_type, self.fsdp_save_policy):
            model_state_dicts = self.vlm.state_dict()
            ...
            if overwatch.is_rank_zero():
                ...
                # (siddk) :: This breaks w/ Sagemaker default permissions (root vs. <user>)... skip?
    ...
    def run_setup(self, run_dir: Path, n_train_examples: int) -> None:
        ...
        # vlm_fsdp_wrapping_policy = size_based_auto_wrap_policy
        # vlm_fsdp_wrapping_policy = None
        ...
        if self.enable_mixed_precision_training and self.mixed_precision_dtype == torch.bfloat16:
            ...
            if self.stage not in {"full-finetune", "vla-full-train", "vla-sandwich-train"}:
                ...
                self.vlm.vision_backbone.to(dtype=self.vlm.vision_backbone_half_precision_dtype)
        ...
        # only print the model once
        if overwatch.is_rank_zero():
            overwatch.info(f"Fully Sharded Data Parallel (FSDP) Strategy =>> Wrapped Model:\n{self.vlm}\n") 

        # Gradient Checkpoint Setup
        # (siddk) :: Gradient Checkpointing is currently disabled due to FSDP incompatibility!
        if self.enable_gradient_checkpointing and False:
