...
class Mixture(nn.Module):
    def __init__(self, config):
        ...
        if config.use_final_norm:
            self.norm = SmolVLMTextRMSNorm(
                config.hidden_size,
                eps=config.rms_norm_eps,
            )
        
        # init weights for action expert, vlm and projector will be covered by loading pre-trained weights
    ...
    def _init_weights(self, module):
        if isinstance(module, nn.Linear):
            torch.nn.init.normal_(module.weight, mean=0.0, std=math.sqrt(2 / (5.0 * self.config.hidden_size)))
...
class MixtureDecoderLayer(nn.Module):
    def __init__(self, config):
        ...
        self.input_layernorm = SmolVLMTextRMSNorm(
            config.hidden_size,
            eps=config.rms_norm_eps,
        )
        self.post_attention_layernorm = SmolVLMTextRMSNorm(
            config.hidden_size,
            eps=config.rms_norm_eps,
        )
...
class MixtureAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.rotary_emb = SmolVLMTextRotaryEmbedding(
            self.head_dim,
